...
public class Mesh(Random rand) {
    ...
    public void Subdivide(int iterations = 1) {
        for (int i = 0; i < iterations; i++) {
            List<Vector3> newVertices = [.._vertices];
            ...
            for (int j = 0; j < _triangles.Count; j += 3) {
                ...
                int m1 = GetMidpointIndex(i1, i2, newVertices, edgeMidpoints);
                int m2 = GetMidpointIndex(i2, i3, newVertices, edgeMidpoints);
                int m3 = GetMidpointIndex(i3, i1, newVertices, edgeMidpoints);
    ...
    public void GenerateIcoSphere(float radius, int recursionLevel) {
        ...
        for (int i = 0; i < recursionLevel; i++) {
            List<Vector3Int> faces2 = [];
            foreach (var tri in faces) {
                // replace triangle by 4 triangles
                int a = GetMiddlePoint(tri.x, tri.y, ref newVertices, ref middlePointIndexCache, radius);
                int b = GetMiddlePoint(tri.y, tri.z, ref newVertices, ref middlePointIndexCache, radius);
                int c = GetMiddlePoint(tri.z, tri.x, ref newVertices, ref middlePointIndexCache, radius);
    ...
    public void GeneratePlane(Vector3 dimensions, int subdivisions) {
        ...
        for (int i = 0; i < initialTriangles.Length; i += 3) {
            triangles.Add(initialTriangles[i + 2] + vertexCount);
            triangles.Add(initialTriangles[i + 1] + vertexCount);
            triangles.Add(initialTriangles[i] + vertexCount);
        }
    ...
    private static int GetMidpointIndex(int i1, int i2, List<Vector3> vertices, Dictionary<string, int> edgeMidpoints) {
        string edgeKey = i1 < i2 ? i1 + "_" + i2 : i2 + "_" + i1;

        if (edgeMidpoints.TryGetValue(edgeKey, out int index)) return index;
    ...
    public void ApplyVoronoiPattern(
        int siteCount,
        float craterDepth,
        float craterRimHeight,
        Color craterBottomColor,
        Color craterRimColor
    ) {
        CalculateBounds();
        ...
        for (int i = 0; i < siteCount; i++) {
            Vector3 seed = new Vector3(
                _bounds.min.x + rand.GetFloat() * _bounds.size.x,
                _bounds.min.y + rand.GetFloat() * _bounds.size.y,
                _bounds.min.z + rand.GetFloat() * _bounds.size.z
            );
            seeds.Add(seed);
        }
